...
* [`src/financials_front/`](src/financials_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/financials_front/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `financials_front-front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/financials_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns financials_front-front.example
  (:require [financials_front-front.config :as config])
